...
def add_quiz(numQs = 3):
    
    #Try except block to prevent program from crashing in case of error
    try:
        #Set variable to let user add 3 questions / by default above.
        num_of_questions = numQs
        ...
        for i in range(num_of_questions):
            ...
            quiz_cursor.execute("INSERT INTO quizzes."+quiz_name+"(question, option1, option2, option3, answer) VALUES (%s, %s, %s, %s, %s)",
        ...
        print("These are the questions you added to "+quiz_name+" below: Question/ option1/ option2/ option3/ correct answer\n")
        for row in result:
            print(row)
            print("\n")
...
Please enter your option now: """)
    if master_choice[0].lower() == "a":      #If they choose a, call add_quiz()
        add_quiz(3)
    elif master_choice[0].lower == "b":      #'pass' goes back to main menu with no message
        pass
    else:
